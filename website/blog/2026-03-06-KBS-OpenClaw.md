...
먼저 로컬 **NVIDIA DGX Spark** 하드웨어에서 OpenClaw 기반 개인 에이전트를 생성하고 실행했습니다. 에이전트는 [**“Jinu(진우)”**](https://www.moltbook.com/u/JinuAI)라는 이름으로 자체 아이덴티티와 사용자 정보를 구성했으며, Moltbook 시스템에 성공적으로 등록되었습니다.
...
이는 OpenClaw 기반 관찰자 에이전트가, Moltbook 규칙 범위 안에서는, 일반 사용자 계정과 동일한 방식으로 활동할 수 있음을 확인해주었습니다.
...
이 단계에서 진우 에이전트는 다음 행동을 수행할 수 있었습니다:
...
## 4. DM 정책 및 에이전트 간 직접 소통

Moltbook의 DM 시스템(Direct Message)은 기본적으로 개방되어 있지 않습니다. DM 대화는 **DM 요청**을 보내고 대상 에이전트의 승인을 받아야만 시작될 수 있습니다.
...
기본적인 게시/댓글/DM 자동화 기능만으로는 관찰 에이전트의 행동 폭이 제한되기 때문에, [**ClawHub**](https://clawhub.ai)를 통한 스킬 학습도 함께 실험했습니다.
이 파트의 목적은 다음 두 가지였습니다:

1. **역량 확장**: 추가 개발 없이도 에이전트가 재사용 가능한 능력(구조화된 사고, 문체 제어, 도메인 보조 등)을 빠르게 획득할 수 있는지 검증
2. **운영 가능성 평가**: 자동화 파이프라인에서 스킬을 반복적으로 사용할 때 발생하는 **토큰 비용, 지연(레이턴시), 안정성**을 실제 관점에서 확인  
   (관찰 에이전트는 장시간·반복 실행을 전제로 하기 때문)
...
일부 스킬은 학습/로드에 실패하거나, 학습되더라도 이후 사용이 안정적이지 않았습니다. 예를 들어:
...
또한 가장 큰 제약은 **비용 및 효율성 문제**였습니다. 많은 ClawHub 스킬이 긴 텍스트 위주의 `SKILL.md` 지침 파일 형태로 제공되는데, 기술적으로는 사용 가능하더라도 운영 관점에서는 다음 문제가 발생했습니다:

- 스킬 지침이 종종 매우 김
- 호출 시 토큰 사용량이 크게 증가
- API 기반 운영에서 반복 사용 시 비용이 커짐

ClawHub 기반 스킬 학습은 추가 엔지니어링 없이 에이전트 기능을 빠르게 확장할 수 있는 유효한 방법이었습니다. 다만 현재의 스킬 패키징 방식(`SKILL.md` 중심)은 **반복 실행이 많은 프로덕션형 자동화**에선 토큰 오버헤드가 병목이 될 수 있습니다.  
따라서 장기 운영을 전제로 하는 관찰 에이전트에는 스킬의 **압축, 모듈화, 선택적 로딩(필요 시에만 주입)** 등의 최적화가 필요합니다.

---

## 8. 로컬 모델 vs. API 기반 모델 비교

로컬과 API 기반 에이전트 운용을 비교하기 위해, 진우 에이전트용으로 **NVIDIA DGX Spark** 머신에 로컬 **GLM-4.7-Flash** 모델을 설치했습니다.
...
이는 에이전트의 활동 반경이 예상보다 훨씬 넓었음을 의미합니다. 전용 에이전트 워크스페이스로 제한되는 대신, 두 설정 모두 더 넓은 로컬 파일 시스템을 가시 범위로 두고 동작했습니다. 결과적으로 프롬프트 인젝션, 탈옥, 또는 적대적 에이전트-대-에이전트 조작이 성공할 경우 OpenClaw 런타임과 무관한 민감 파일까지 노출될 수 있습니다.
